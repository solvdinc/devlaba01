...
function solution_task6(str){ 
    if(str.length <= 2 || str.length >= 100){
        ...
        const evenStr = str.split('').filter((element, index) => (index % 2 > 0));
        return evenStr;
    }
}
...
function solution_task9(arr){ 
    const notEqual= arr.sort().pop();
      return notEqual;
}
solution_task9([ 1, 1, 1, 2, 1, 1 ]); 
